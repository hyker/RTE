...
LETSENCRYPT_STAGING="true"   # change to "false" for production
...
rm -f vm-disk-staging.qcow2
qemu-img create -f qcow2 -b "$CLOUD_IMAGE" -F qcow2 vm-disk-staging.qcow2 4G
...
cp "$PROJECT_DIR"/index.html \
   ...
   "$WEBSTAGING"/
rm -f webroot-staging.iso
genisoimage -quiet -o webroot-staging.iso -V WEBROOT -r -J "$WEBSTAGING"
rm -rf "$WEBSTAGING"
...
rm -f seed-staging.iso
cloud-localds seed-staging.iso /tmp/ci-user-data /tmp/ci-meta-data
rm -f /tmp/ci-user-data /tmp/ci-meta-data

echo ""
echo "Build complete. Files:"
echo "  vm-disk-staging.qcow2  - VM disk (overlay on cloud image)"
echo "  seed-staging.iso        - cloud-init configuration"
echo "  webroot-staging.iso     - web content"
echo ""
echo "Run: HTTPS_PORT=8446 VM_NAME=quote-verifier-staging ./boot.sh"
